...
public class tiny_gp {
    ...
    static final int
    ADD=110,SUB=111,MUL=112,DIV=113, FSET_START=ADD,FSET_END=DIV;
    ...
    static final int
    MAX_LEN=10000,
    POPSIZE=100000,DEPTH=5,GENERATIONS=100,TSIZE=2;
    public static final double
    PMUT_PER_NODE=0.05,CROSSOVER_PROB=0.9;
...
void setup_fitness(String fname) {
    try {
        ...
        if (varnumber + randomnumber >= FSET_START)
            System.out.println("too many variable and contanst000");
...
int print_indiv(char [] buffer, int buffercounter){
        ...
        if (buffer[buffercounter]<FSET_START){
            if (buffer[buffercounter]<varnumber)
                System.out.print("X"+(buffer[buffercounter]+1)+"_");
            else
        ...
        switch (buffer[buffercounter]){
            case ADD:
                System.out.print("(");a1=print_indiv(buffer,++buffercounter);
...
void stats ( double [ ] fitness , char [ ] [ ] pop , int gen ){
    ...
    favgpop /= POPSIZE;
   System.out.print("Generation="+gen+"avf_fitness="+(-favgpop)+"best_fitness="+(-fbestpop)+"avg_size="+avg_len+"nbest_individual:");
...
char [ ] mutation ( char [ ] parent , double pmut ) {
    ...
    void print_parms (){
        ...
        System.out.print("seed"+seed+"nmaxlien"+MAX_LEN+"NPOPSIZE="+POPSIZE+"ndepth="+
        DEPTH+"crossover_prob="+CROSSOVER_PROB+"pmut+per_node="+PMUT_PER_NODE+
        "MINRANDOM="+minrandom+"MAXRANDOM"+maxrandom+"ngeneration="+GENERATIONS+
        "ntsize="+TSIZE);
